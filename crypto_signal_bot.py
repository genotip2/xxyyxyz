...
def generate_signal(pair):
    ...
    if pair not in ACTIVE_BUYS:    
        ...
        best_exit_ok, best_exit_msg = is_best_exit_from_data(data)    
        if best_exit_ok:
            # Simpan data entry sebelum dihapus
            entry_data = ACTIVE_BUYS[pair]
            print(f"✅ Pair {pair} dihapus dari active buys karena best exit terpenuhi.")
            send_telegram_alert("SELL", pair, current_price, f"BEST EXIT: {best_exit_msg}\nProfit: {profit_from_entry:+.2f}%", entry_analysis, entry_data)
        ...
        if holding_duration > timedelta(days=MAX_HOLD_DURATION_DAYS):    
            ...
            send_telegram_alert("EXPIRED", pair, current_price, f"Durasi hold: {holding_duration.days} hari", entry_analysis, entry_data)
        ...
        if profit_from_entry <= -STOP_LOSS_PERCENTAGE:    
            ...
            send_telegram_alert("STOP LOSS", pair, current_price, "Stop loss tercapai.", entry_analysis, entry_data)
        ...
        if data_active.get('trailing_stop_active', False):    
            ...
            if current_price < trailing_stop_price:    
                ...
                send_telegram_alert("TRAILING STOP", pair, current_price, f"Harga turun ke trailing stop: {trailing_stop_price:.8f}", entry_analysis, entry_data)
...
##############################
def send_telegram_alert(signal_type, pair, current_price, details="", entry_analysis=None, entry_data=None):
    ...
    # Jika sinyal bukan BUY, gunakan data entry (baik dari parameter entry_data atau dari ACTIVE_BUYS jika tersedia)
    if signal_type != "BUY":
        if entry_data is None and pair in ACTIVE_BUYS:
            entry_data = ACTIVE_BUYS[pair]
        if entry_data:
    ...
    # Tambahan info indikator jika tersedia
    if entry_analysis is None:
        entry_analysis = analyze_pair_interval(pair, TIMEFRAME_ENTRY)
    if entry_analysis:
